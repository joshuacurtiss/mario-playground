...
import { GameObj, Rect, Vec2 } from 'kaplay';
...
export function isPiranha(obj: GameObj): obj is Piranha {
   return isEnemy(obj) && PIRANHA_KINDS.some(kind => obj.is(kind));
}
...
function piranha(opts: Partial<PiranhaCompOpt> = {}): PiranhaComp {
   ...
   let timer = 0;
   let shot = false;
   let visibleRect: Rect | undefined;
   ...
   return {
      ...
      getVisibleRect() {
         return visibleRect?.clone();
      },
      add(this: Piranha) {
         if (!debug) return;
         const drawVisibleRect = k.onDraw(() => {
         ...
         this.onDestroy(() => drawVisibleRect.cancel());
      },
      fixedUpdate(this: Piranha) {
         ...
         if (parent && isGameObjWithRect(parent)) {
            const parentMaskRect = getRectWorldBounds(parent);
            // Use actual area geometry so piranha bounds keep rectangular proportions.
            const piranhaRect = this.worldArea().bbox();
            visibleRect = parentMaskRect ? intersectRects(piranhaRect, parentMaskRect) : undefined;
         } else {
            visibleRect = undefined;
         }
...
export interface PiranhaOpt extends PiranhaCompOpt {
   kind: PiranhaKind;
   type: PiranhaType;
   points: number;
}
